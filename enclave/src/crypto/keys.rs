...
pub fn pk_from_sk(sk: &SgxPrivateKey) -> SgxResult<PublicKey> {
...
use ed25519_dalek::{PUBLIC_KEY_LENGTH,KEYPAIR_LENGTH,Keypair,Signer};
use crypto::CryptoResult;

pub fn sign_submission(msg: &UserSubmissionMessage, ssk: &SgxPrivateKey) -> CryptoResult<(SignatureBytes, PublicKey)> {
    let dig = msg.digest();
