...
static sgx_status_t SGX_CDECL sgx_test_aggregator(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	test_aggregator();
...
static sgx_status_t SGX_CDECL sgx_ecall_scheduling(void* pms)
{
	...
	ms->ms_retval = ecall_scheduling((const void*)_tmp__prev_msg, _tmp__state, _tmp__new_msg);
...
static sgx_status_t SGX_CDECL sgx_ecall_aggregate(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const UserMessage_C* _tmp__message = ms->ms__message;
	const AggregatedMessage_C* _tmp__cur_agg = ms->ms__cur_agg;
	AggregatedMessage_C* _tmp__new_agg = ms->ms__new_agg;



	ms->ms_retval = ecall_aggregate((const UserMessage_C*)_tmp__message, (const AggregatedMessage_C*)_tmp__cur_agg, _tmp__new_agg);
...
SGX_EXTERNC const struct {
	...
	6,
	{
		...
		{(void*)(uintptr_t)sgx_test_aggregator, 0, 0},
