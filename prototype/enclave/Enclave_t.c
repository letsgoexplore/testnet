...
typedef struct ms_ecall_scheduling_t {
  int ms_retval;
  const SchedulingMessage_C* ms__prev_msg;
  SchedulingState_C* ms__state;
  SchedulingMessage_C* ms__new_msg;
...
static sgx_status_t SGX_CDECL sgx_test_all(void* pms)
{
  sgx_status_t status = SGX_SUCCESS;
  if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
  test_all();
...
static sgx_status_t SGX_CDECL sgx_ecall_scheduling(void* pms)
{
  ...
  sgx_status_t status = SGX_SUCCESS;
  const SchedulingMessage_C* _tmp__prev_msg = ms->ms__prev_msg;
  SchedulingState_C* _tmp__state = ms->ms__state;
  SchedulingMessage_C* _tmp__new_msg = ms->ms__new_msg;

  ms->ms_retval = ecall_scheduling(
      (const SchedulingMessage_C*)_tmp__prev_msg, _tmp__state, _tmp__new_msg);
...
static sgx_status_t SGX_CDECL sgx_ecall_aggregate(void* pms)
{
  ...
  ms->ms_retval = ecall_aggregate((const DCNetSubmission_C*)_tmp__message,
                                  (const AggregatedMessage_C*)_tmp__cur_agg,
                                  _tmp__new_agg);
  ...
  struct {
    ...
    uint8_t is_switchless;
...
} g_ecall_table = {6,
                   {
                       ...
                       {(void*)(uintptr_t)sgx_ecall_get_mr_enclave, 0, 0},
                       {(void*)(uintptr_t)sgx_TestScheduling, 0, 0},
                       {(void*)(uintptr_t)sgx_test_all, 0, 0},
                       {(void*)(uintptr_t)sgx_ecall_scheduling, 0, 0},
