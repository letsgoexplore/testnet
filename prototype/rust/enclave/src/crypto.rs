use interface::*;
use crate::sgx_types::{SGX_HMAC256_MAC_SIZE,SGX_HMAC256_KEY_SIZE};
...
pub fn derive_round_secret(round: u32, server_secrets: &Vec<ServerSecret>) -> CryptoResult<RoundSecret> {
    // for each key, run KDF, then xor together all outputs
    match server_secrets.iter().map(|s| {
        kdf_hmac(s, round)
    }).collect::<CryptoResult<Vec<RoundSecret>>>() {
        Ok(round_secrets) => {
            Ok(round_secrets.iter().fold(RoundSecret::zero(), |acc, s| {
                RoundSecret::xor(&acc, s)
            }))
        }
        Err(e) => Err(e)
    }
...
pub fn sign_dc_message(msg: &SignedUserMessage, tee_prv_key: PrvKey) -> CryptoResult<SignedUserMessage> {
